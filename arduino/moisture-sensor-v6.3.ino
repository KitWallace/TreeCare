...
#define WARMUP_TIME_TO_SLEEP 45

// refresh interval
#define uS_TO_S_FACTOR  1000000ULL     /* Conversion factor for micro seconds to seconds - cast as ULL to allow long sleeps  */        
#define REPORTING_TIME_TO_SLEEP 120*60  /* Time ESP32 will go to sleep (in seconds)  */

#define AVERAGE_RUN_TIME  23 // 23 seconds is the average time from wake up , sensor readings, GSM startup, transmission and HTTP communication
...
bool GSM_start() {
  ...
  if (! setPowerBoostKeepOn(1) ) {
    SerialMon.println("IP5306 KeepOn fail");
    return false;
  }
...
void setup() {
 ...
 // set the sleep time depending on the phase of operation 
    ...
    sleep_time = sleep_time- AVERAGE_RUN_TIME;
